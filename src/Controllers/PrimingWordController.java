...
public class PrimingWordController implements Initializable
{
    ...
    long init, click;
    private ArrayList<Word> niu = new ArrayList();
    FamiliarityWordController familiarityWord = new FamiliarityWordController();
    
    public PrimingWordController()
    {
        niu = familiarityWord.getWordList();
    }
...
//	}
  	...
  	public Word getWordByName(String wordName)
    {
        ...
        catch(Exception e)
        {
            ...
            System.out.println("No se pudo conectar a la base de datos o no pudo realizar el query");
        }
	...
	public String[] selectTwoAnswerOptions(Word hiddenWord)
    {
        connect = dbConnection.getConnection();
...
//					"where category = 'Neutra' and quantitySyllables = ? and initialLetter = ?" +
					"where category = 'Neutra' and quantitySyllables = ? " +
					"order by random()\r\n" + 
					"limit 1;";
			try
            {
...
//						"where category = 'Negativa' and quantitySyllables = ? and initialLetter = ?" +
					 	"where category = 'Negativa' and quantitySyllables" +
						"order by random()\r\n" + 
						"limit 1;";
            try
            {
...
//								"where category = 'Positiva' and quantitySyllables = ? and initialLetter = ?" +
							 "where category = 'Positiva' and quantitySyllables = ? " +
								"order by random()\r\n" + 
								"limit 1;";
					try {
                        PreparedStatement preparedStatement2 = connect.prepareStatement(sql2);
